"""HTTP 处理：JSON 路由、错误映射、演示页面与启动入口。"""
...
from app import BASE_DIR, DEFAULT_DB, BusinessError, ReviewStore


class ReviewHandler(BaseHTTPRequestHandler):
    server_version = "AcademicReview/1.1"
    ...
    def _dispatch(self, method: str) -> None:
        ...
        if len(parts) >= 3 and parts[:2] == ["api", "papers"]:
            ...
            if len(parts) == 4 and parts[3] == "backfill" and method == "POST":
                return self._send(201, store.run_backfill(self._user_id(), paper_id))
